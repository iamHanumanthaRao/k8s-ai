...
if infra == 'Kubernetes':
    st.write('Selected Infra is Kuberenetes')
    showRadio()
    st.title('Alfred - AI ')
...
def writePods(info):
    ...
    components.v1.html(rendered_html, height=600)    
...
def getInfo(cmd):
    try:
        ...
        st.write("Please conect to the right VPN or check your Internet Connection or I am not there yet to understand the question :-)")

def gptInfo(input_text):
    keyWords = ["erase", "delete", "remove", "Delete", "DELETE"]
    first_input_prompt=PromptTemplate(
        input_variables=['command'],
        template="Command in k8s for {command} respond with command as plain text"
    )
    ## OPENAI LLMS
    ...
    if input_text:
        for key in keyWords:
            if key in input_text:
                st.write("I am not permitted to perform this action")
                sys.exit()                   
